...
#include <rbcar_steering_controller/MotorDrive.h>
#include <math.h>
...
void MotorDrive::ReadyState(){
	...
	if((dsDesiredStatus != dsStatus) && (diff > 50000)){	
		...
		} else if(dsDesiredStatus == READY_TO_SWITCH_ON){
			switch(dsStatus){
				case OPERATION_ENABLED:
					ROS_INFO("%s::ReadyState: Node %d: OPERATION_ENABLED to READY_TO_SWITCH_ON", sComponentName.c_str(), CanId);
					Send(CanId, SHUTDOWN_MSG, WRITE, 0);					
					//Delay();
				break;;
				case OPERATION_DISABLED:
					ROS_INFO("%s::ReadyState: Node %d: OPERATION_DISABLED to READY_TO_SWITCH_ON", sComponentName.c_str(), CanId);
					Send(CanId, SHUTDOWN_MSG, WRITE, 0);					
					//Delay();
				break;;

				case SWITCH_ON_DISABLED:
					ROS_INFO("%s::ReadyState: Node %d: SWITCH_ON_DISABLED to READY_TO_SWITCH_ON", sComponentName.c_str(), CanId);
					Send(CanId, SHUTDOWN_MSG, WRITE, 0);
					//Delay();
				break;

				case FAULT:
					ROS_INFO("%s::ReadyState: Node %d: FAULT to SWITCH_ON_DISABLED", sComponentName.c_str(), CanId);
					Send(CanId, RESET_MSG, WRITE, 0);
				break;
				default:
					ROS_INFO("%s::ReadyState: Node %d: OTHERS to SWITCH_ON_DISABLED", sComponentName.c_str(), CanId);
					Send(CanId, DISABLE_VOLT_MSG, WRITE, 0);
					//Delay();
				break;
			}
		}
...
void MotorDrive::ProcessPositionValue(TPCANMsg msg){
    ...
    iPositionRef = aux_position;
    //ROS_INFO("MotorDrive::ProcessPositionValue");
}
...
/*! \fn ReturnValue MotorDrive::ConfigureNodeGuardTimer()
